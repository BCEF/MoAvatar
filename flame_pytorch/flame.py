...
#SUMO
from .flame_subdivision import subdivide_mesh, load_flame_obj
...
class FLAME(nn.Module):
    ...
    def forward_geo(
        ...
        if transl is None:
            transl = torch.zeros(batch_size, 3).cuda()
        if neck_pose is None:
            neck_pose = torch.zeros(batch_size, 3).cuda()
        if pose_params is None:
            pose_params = torch.zeros(batch_size, 6).cuda()
        if eye_pose is None:
        ...
        if scale_factor is None:
            scale_factor = torch.ones(batch_size, 1).cuda()
        betas = torch.cat(
            [shape_params, self.shape_betas, expression_params, self.expression_betas],
            dim=1,
        )
    ...
    def forward_geo_subdivided(
        ...
        transl=None,
        scale_factor=1,
        subdivision_level=2,  # 新增参数：细分级别
    ):
        """
        修改后的forward_geo函数，支持顶点细分
        
        Args:
            subdivision_level: 细分级别，每级增加约4倍顶点数
            其他参数与原函数相同
        
        Returns:
            vertices: [N, V_new, 3] 细分后的顶点，数量约为原来的4^subdivision_level倍
        """
        batch_size = shape_params.shape[0]
        I = matrix_to_rotation_6d(torch.cat([torch.eye(3)[None]] * batch_size, dim=0).cuda())
        ...
        if subdivision_level > 0:
            # 加载FLAME面片信息
            _, faces, uvs = load_flame_obj('model/FlameMesh.obj')
            
            # 执行mesh细分
            vertices = subdivide_mesh(vertices, faces, subdivision_level)
